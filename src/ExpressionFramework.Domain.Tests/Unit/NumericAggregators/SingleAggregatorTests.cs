...
public class SingleAggregatorTests
{
    ...
    public void Aggregate_Returns_Invalid_When_SecondValue_Could_Not_Be_Converted_To_Correct_Type()
    {
        ...
        result.ErrorMessage.Should().Be("Could not convert SecondExpression to Single. Error message: The input string 'unsupported type' was not in a correct format.");
        result.Value.Should().BeNull();
    }
