...
[ExpressionDescription("Gets the first value from the (enumerable) context value, optionally using a predicate to select an item")]
[UsesContext(true)]
...
[ReturnValue(ResultStatus.Ok, typeof(object), "Value of the first item of the enumerable that conforms to the predicate", "This will be returned in case the enumerable is not empty, and no error occurs")]
...
public partial record FirstExpression
{
    public override Result<object?> Evaluate(object? context)
        => EnumerableExpression.GetScalarValueWithoutDefault(context, Predicate, results => results.First(), results => results.First(x => x.Result.Value).Item);
