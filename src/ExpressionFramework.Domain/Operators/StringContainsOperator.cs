...
[ReturnValue(ResultStatus.Ok, typeof(bool), "true or false", "True when both values are string, and the left value contains the right value, otherwise false")]
[ReturnValue(ResultStatus.Invalid, "Empty", "Left value or right value are not of type string")]
public partial record StringContainsOperator
{
    protected override Result<bool> Evaluate(object? leftValue, object? rightValue)
        => leftValue is string leftString && rightValue is string rightString
            ? Result<bool>.Success(leftString.IndexOf(rightString, StringComparison.CurrentCultureIgnoreCase) > -1)
