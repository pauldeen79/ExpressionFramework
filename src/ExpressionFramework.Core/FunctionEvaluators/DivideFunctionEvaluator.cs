...
public class DivideFunctionEvaluator : IFunctionEvaluator
{
    ...
    {
        result = null;
        if (!(function is DivideFunction divideFunction))
        {
            return false;
        }

        var divideByValue = evaluator.Evaluate(sourceItem, divideFunction.DivideByExpression);
