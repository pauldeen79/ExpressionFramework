            ...
            .OnSuccess(result =>
                ...
                .AddMethods(
                    ...
                    new MethodBuilder()
                        ...
                        .AddStringCodeStatements($"return new {typeof(ResultDictionaryBuilder).FullName}(){GetFunctionArgumentsAddString(x)}.Build().OnSuccess(results => {typeof(Result).FullName}.Success<ExpressionFramework.Core.Abstractions.IOperator>(new {x.WithoutInterfacePrefix()}({GetFunctionArgumentsGetString(x)})));")
                )
